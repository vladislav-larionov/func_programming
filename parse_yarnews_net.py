...
class YarnewsNetParser(NewsSiteParser):
    ...
    def retrieve_article_text(self, link: str) -> str:
        ...
        """
        full_text = ''
        text_parts = super().get_html(link).find('div', class_="text").find_all('p')
    ...
    def parse_news_items(self, items: list, earliest_date: str) -> list:
        """
        ...
        for item in items:
            ...
            full_text = self.retrieve_article_text(link)
            date_time = datetime.strptime(item.find('span', class_="news-date").get_text(),
                                          "%d.%m.%Y в %H:%M")
            print(f'{date_time.strftime("%H:%M:%S, %Y-%m-%d")} | {title}', file=sys.stderr)
            categories = list()
    ...
    def parse(self, earliest_date: datetime) -> list:
        ...
        while more_news_exists:
            request = super().get_html('https://www.yarnews.net/news/chronicle/ajax/' + str(news_loaded) + '/')
